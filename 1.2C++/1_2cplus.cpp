...
public:
    ...
    SequenceLocation(InflectionLocation Point) {
        Sequence.push_back(InflectionLocation());
        Sequence.push_back(Point);
    }
    SequenceLocation(int i=300)
...
public:
    ...
    SequenceBox(SizeOfBox Box) {
        Sequence.push_back(SizeOfBox());
        Sequence.push_back(Box);
    }
    SequenceBox(int i=100)
...
public:
    ...
    SequencePoint(InflectionPoint Point) {
        Sequence.push_back(InflectionPoint());
        Sequence.push_back(Point);
    }
    SequencePoint(int i=500)
...
double 累计概率连续伤害(枪械 枪械实体, 目标总体 目标实体,玩家技术 玩家实体, int 运算精度,double& 击杀时间,int 目标血量=0,bool 详细内容开关=false) {//无压枪 目标无位移 
    double 临时伤害总和变量 = 0;
    double 时间变量 = 0;
    double 命中概率 = 0;
    int i;
...
void 改进TTK计算(枪械 枪械实体, 目标总体 目标实体, 玩家技术 玩家实体, int DPS运算精度,int 运算次数) {
    ...
    for (int i = 0; i < 运算次数; i++) {
        改进TTK = 0;
        临时血量 = 目标实体.目标血量;
        单弹夹总输出 = 累计概率连续伤害(枪械实体, 目标实体, 玩家实体, DPS运算精度,击杀时间, 临时血量);
        for (; 单弹夹总输出 < 临时血量;) {
...
void 枪械修改(枪械& 枪械实体) {
    for (;;) {
        char 输入 = 0;
        int 临时int变量 = 0;
        double 临时double变量 = 0.0;
        int 临时数组变量 = 0;
        double x = 0, y = 0, time = 0;
        bool 循环变量 = 0;
        cout << "输入a进行武器伤害修改 输入b进行武器射速修改 输入c进行单弹夹耗光时间修改 输入d进行子弹散布参数修改 输入e进行换弹时间修改 " << endl;
        cout << "输入f进行后座修改 输入g查看数据 输入h查看后座图像 输入其他退出(目前子弹衰减放大倍率 放大倍率 持枪移动速度不可在程序内修改):" << endl;
        cin >> 输入;
        cout << endl;
        switch (输入) {
        case 'a': {
        ...
        case'h': {
            SequencePoint 压枪拐点(1);
            SequenceLocation 目标移动(1);
            SequencePoint 跟枪拐点(1);
            SequenceBox 目标大小(1);
            压枪拐点.Sequence.push_back(InflectionPoint(0,0,100));
            目标移动.Sequence.push_back(InflectionLocation(0,0,10,100));
            跟枪拐点.Sequence.push_back(InflectionPoint(0, 0, 100));
            目标大小.Sequence.push_back(SizeOfBox(0, 0, 100));
            枪械实体.枪械子弹序列.屏幕拐点位置_转速度函数();
            显示函数(枪械实体.枪械子弹序列, 压枪拐点, 目标移动, 跟枪拐点, 目标大小, 枪械实体.子弹散布参数);
            break;
        }
        default: {
            return;
...
void 目标修改(目标总体& 目标实体) {
    for (;;) {
        ...
        bool 循环变量 = 0;
        cout << "输入a进行目标血量修改 输入b进行目标移动修改 输入c进行目标大小修改 输入d查看数据 输入e查看移动图像 输入其他退出" << endl;
        cin >> 输入;
        cout << endl;
        switch (输入) {
        case 'a': {
        ...
        case'e': {
            for (临时数组变量 = 1; 临时数组变量 < 目标实体.移动序列.Sequence.size() - 1; 临时数组变量++) {
                if (目标实体.移动序列.Sequence[临时数组变量].Time == 0) {
                    break;
                }
            }
            SequencePoint 后坐力拐点(1);
            SequencePoint 压枪拐点(1);
            SequencePoint 跟枪拐点(1);
            后坐力拐点.Sequence.push_back(InflectionPoint(0, 0, 目标实体.移动序列.Sequence[临时数组变量 - 1].Time));
            后坐力拐点.Sequence.push_back(InflectionPoint(0, 0, 0));
            压枪拐点.Sequence.push_back(InflectionPoint(0, 0, 100));
            跟枪拐点.Sequence.push_back(InflectionPoint(0, 0, 100));
            目标实体.移动序列.实际拐点位置_转速度函数();
            显示函数(后坐力拐点, 压枪拐点,目标实体.移动序列, 跟枪拐点, 目标实体.目标大小序列, 0);
            break;
        }
        default: {
            return;
...
void 交互(枪械& 枪械实体, 目标总体& 目标实体, 玩家技术& 玩家实体) {
    for (;;) {//相当于while(true)
        char 输入 = 0;
        int 临时int变量 = 0, 临时运算次数变量 = 0;
        double 击杀时间 = 0;
        ...
        case'd': {
            ...
            case'a': {
                ...
                cin >> 临时int变量;
                累计概率连续伤害(枪械实体, 目标实体, 玩家实体, 临时int变量, 击杀时间, true);
                break;
            }
            case'b': {
                SequencePoint 压枪序列 = SequencePoint(100);
